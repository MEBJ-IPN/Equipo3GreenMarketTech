...
def regresar():
    pantallaDuenoV.pack_forget()
    import incio_I
    incio_I.pantallaCuentaDueno(ventana, pantallaDuenoV)

#Pantalla principal de dueño
def pantallaDueno(ventana, frameQ):
    global pantallaDuenoV
    #Frame usuarios
    pantallaDuenoV = ctk.CTkFrame(ventana, bg_color="#D9D9D9")
    pantallaDuenoV.pack(fill="both", expand=True)
    ...
    bgLab = ctk.CTkLabel(pantallaDuenoV, image=bgP, text="", anchor="center")
    bgLab.place(relwidth=1, relheight=1, anchor="center", relx=0.5, rely=0.5)

    cerrarSesion = ctk.CTkButton(pantallaDuenoV, text="Cerrar sesión", font=("Roboto", 25, "bold"), border_width=0, corner_radius=30, bg_color="#92ADA7", image=flechaIcono, compound="left", command=lambda:regresar())
